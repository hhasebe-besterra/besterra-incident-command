...
const META = {
  ...
  urgency:{H:'高 / 即時',M:'中 / 1〜3日',L:'低 / 任意'},
  ...
  switch(action){
    ...
    case 'create': {
      ...
      const ins={ type, title:params.title, description:params.description||'',
        ...
        received_at:params.received_at||null, due_date:params.due_date||null, notify:params.notify===undefined?true:!!(+params.notify),
    ...
    case 'update': {
      ...
      if(params.due_date!==undefined){ const nv=params.due_date||null, ov=cur.due_date||null;
        if(nv!==ov){ set.due_date=nv; changes.push('クローズ予定日 → '+(nv||'—')); } }
...
function incRows(list){
  return list.map(i=>`<tr data-id="${i.id}"><td class="code">${esc(i.code)}</td><td class="c-chip">${typeChip(i.type)}</td><td class="c-chip">${prioBadge(i.priority)}</td><td class="c-chip">${statusChip(i.status)}</td>
      ...
      <td class="t-meta c-kv" data-label="最終更新日時" title="${esc(ago(i.updated_at))}">${esc(fmt(i.updated_at))}</td></tr>`).join('');
}
...
function sortList(list,key,dir){
  ...
  return [...list].sort((a,b)=>{ const x=val(a),y=val(b); let c=x<y?-1:x>y?1:0;
    ...
    return new Date(b.updated_at||0)-new Date(a.updated_at||0); });   // 同値は更新が新しい順
}
...
async function loadIncidents(){
  ...
  const j=await api('list',p); State.incidents=j.incidents||[]; renderIncList();
}
...
function openNew(){
  ...
  openDrawer(`
   ...
   <div class="dr-body">
     <div class="form-row"><label>種別 / TYPE</label><select class="inp" id="n-type">${opts(m.types,'incident',v=>v.icon+' '+v.label)}</select><div class="type-help" id="n-typehelp"></div></div>
