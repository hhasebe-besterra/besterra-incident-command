...
async function loadDashboard(){
  ...
  const list=(await api('list',{scope:'open'},'GET')).incidents;
...
function renderDashCards(s){
  const tcards=Object.entries(M().types).map(([k,v])=>{
    const open = s.by_type[k]||0;
    return `<div class="card type-card type-${k}">
      <div class="ttl">${v.icon} ${esc(v.label)}（累計）</div>
      <div class="big">${open}</div>
      <div class="sub">${esc(v.desc)}</div></div>`;
  }).join('');
  const pmax=Math.max(1,...Object.values(s.by_priority));
  const pcol={P1:'var(--sev1)',P2:'var(--sev2)',P3:'var(--sev3)',P4:'var(--green)',P5:'var(--sev4)'};
  const prows=Object.entries(s.by_priority).map(([k,v])=>`<div class="mb-row"><span class="lbl">${esc(M().priorities[k].label)}</span>
      <span class="mb-track"><span class="mb-fill" style="width:${v/pmax*100}%;background:${pcol[k]}"></span></span><span class="num">${v}</span></div>`).join('');
  const cmax=Math.max(1,...Object.values(s.by_category));
  const crows=Object.entries(s.by_category).map(([k,v])=>`<div class="mb-row"><span class="lbl">${esc(M().categories[k])}</span>
      <span class="mb-track"><span class="mb-fill" style="width:${v/cmax*100}%;background:var(--cyan)"></span></span><span class="num">${v}</span></div>`).join('');
...
function incTable(list){
  ...
  const rows=list.map(i=>`
    <tr data-id="${i.id}">
      ...
      <td><div class="t-title">${esc(i.title)}</div>
          <div class="t-meta">${esc(M().categories[i.category]||i.category)}${i.affected?' · '+esc(i.affected):''} ${slaTag(i)}</div></td>
      <td class="t-meta">${esc(i.assignee||'未割当')}</td>
      <td class="t-meta" title="${esc(fmt(i.updated_at))}">${ago(i.updated_at)}</td>
    </tr>`).join('');
...
async function openIncident(id){
  ...
  const editPanel = writer ? `
    ...
    <div class="form-2">
      ...
      <div class="form-row"><label>満足度(CSAT)</label><select class="inp" id="e-csat">
        <option value="">未評価</option>${[5,4,3,2,1].map(n=>`<option value="${n}" ${String(i.csat)===String(n)?'selected':''}>${'★'.repeat(n)}（${n}）</option>`).join('')}</select></div>
    </div>
  ...
  if(writer){
    ...
    attachEmpAutocomplete($('#e-assignee'), ()=>State.assignees);
    $('#e-save').onclick=async()=>{
      const p={ id, impact:$('#e-impact').value, urgency:$('#e-urgency').value, status:$('#e-status').value,
        ...
        linked:$('#e-linked').value.trim(), note:$('#e-note').value.trim() };
  ...
  openDrawer(`
   ...
   <div class="dr-body">
     ...
     <div class="form-2">
       ...
       <div class="form-row ac-host"><label>担当</label><input class="inp" id="n-assignee" placeholder="氏名・部署で検索（任意）"></div>
     </div>
     <div class="form-2">
       <div class="form-row"><label>影響範囲 / システム</label><input class="inp" id="n-affected" placeholder="例: 本社7F / メールサーバー"></div>
